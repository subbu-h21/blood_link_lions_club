...
//
// UI-only addition: a landing hub above the search form with four entry
// points (find blood / donor / bank / admin) - routes and SearchForm's
// own logic are unchanged, this only adds navigation chrome around it.
export default async function Home() {
  ...
  return (
    <main className="flex flex-1 flex-col">
      ...
      <section className="mx-auto grid w-full max-w-3xl grid-cols-1 gap-4 px-4 py-14 sm:grid-cols-2">
        <a
          href="#search"
        ...
        <Link
          ...
          className="group flex flex-col gap-2 rounded-2xl border border-ink-100 bg-white p-5 shadow-[var(--shadow-soft)] transition hover:-translate-y-0.5 hover:shadow-[var(--shadow-lift)]"
        >
          <span className="flex h-10 w-10 items-center justify-center rounded-full bg-soil-100 text-soil-700">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path
          ...
          <span className="font-display text-lg font-semibold text-ink-900">{t.bankTitle}</span>
          <span className="text-sm text-ink-500">{t.bankDescription}</span>
        </Link>
        ...
        >
          ...
          <span className="font-display text-lg font-semibold text-ink-900">{t.adminTitle}</span>
          <span className="text-sm text-ink-500">{t.adminDescription}</span>
        </Link>
      </section>
    </main>
  );
