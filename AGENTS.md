...
Supabase Auth (GoTrue) calls MAY run in the browser: `signInWithOtp`, `verifyOtp`, `signInWithPassword`, `updateUser`, `refreshSession`, `signOut`, `getSession`, `onAuthStateChange`. The publishable key is public by design and interactive OTP/password entry requires browser participation.

`resetPasswordForEmail` is deliberately not in this list — not a safety exclusion, but because bank/admin password resets are admin-mediated only (see Conventions: no self-service reset for `app/bank/`, `app/admin/`).
