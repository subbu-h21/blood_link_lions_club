...
## Launch infrastructure + real production data (2026-08-10) — written live, same session as the work

**The app is real and public now: `https://bloodlink.live`.** `prompts/61-launch-infrastructure.md` sections 1-6 all shipped this session (GitHub → hosted Supabase → Vercel+domain → redirect URLs → real accounts → prod push keys), in that order, per the project owner's explicit instruction not to reorder around the still-deferred SMS/OTP decision (section 7 - stays parked, don't raise it unprompted). Full blow-by-blow (CLI mechanics, exact commands, credential-handling notes) lives in that file's own COMPLETED marker and the session transcript; this entry is the durable summary.

**Two real gaps caught mid-flow, not hypothetical:**
1. **The GitHub repo was actually public, not private**, despite that being the explicit choice in section 1 - neither the project owner nor the session verified this after repo creation (GitHub defaults to Public via the web UI). Caught independently (an unauthenticated `curl`/API check), fixed by the project owner, re-verified independently after. Nothing sensitive leaked in the window it was public - the Twilio SID scrub from section 1 had already happened before the first successful push, so the exposed history never contained it.
2. **`supabase/migrations/20260803180000_scheduled_jobs.sql`'s Vault secrets (`cron_site_url`, `cron_secret`) are LOCAL-DEV-ONLY placeholders that get applied verbatim by `supabase db push` to any project, hosted or not** - after section 2's migration push, the hosted project's `pg_cron` jobs were all silently pointed at `http://host.docker.internal:3000`, unreachable from real infrastructure. Fixed via a one-off Management-API SQL call (`vault.update_secret`), deliberately NOT a migration since these two values must differ per-environment. **If this project's scheduled-jobs migration is ever re-applied to a new hosted project, this same fix will be needed again** - there's no automation preventing it, only this note.

**Standing gotcha for anyone running `supabase config push` against this project again**: `config.toml`'s `[auth.sms.test_otp]` (12 hardcoded phone numbers, fixed OTP `123456`, `enable_signup = true`) is a real committed local-dev convenience that becomes a live authentication bypass if pushed to a public project as-is. Every push this session temporarily stripped it (`enable_signup = false`, block removed) before pushing, then `git checkout`-reverted the committed file after - there's no CLI safeguard, only manual discipline. Same treatment needed for `site_url` (must differ between local `127.0.0.1:3000` and the real domain - never commit the real value).

**Real platform-manager account created and live-verified** (`subramanyah65@gmail.com`) - not just trusted from the bootstrap script's own success message: queried `profiles`/`auth.users` directly, then did a real password-grant login against the live Auth API and decoded the returned JWT to confirm `custom_access_token_hook` is genuinely working on the hosted project. Regional admin and bank-staff accounts deliberately NOT created yet - asked the project owner directly, real people aren't lined up. Whoever picks this up next: platform-manager's one-time password only works once (forced reset on first `/ops-control` login, same as every other account tier) - confirm the project owner actually completed that login before assuming the account is in normal use.

**Real e-RaktKosh-sourced blood stock entered for two Sirsi banks, with a real mistake made and caught mid-task, worth the full honest account.** The project owner asked for the two Sirsi banks' current stock to be looked up on e-RaktKosh's public "Blood Availability" search (a legitimate, publicly-published government feature - not the same as editing/submitting data there, which was correctly declined earlier in the same conversation as needing real blood-bank-in-charge credentials, not something to delegate to an agent). Automated with a real Playwright browser session (`web-push`-adjacent tooling already in `frontend/package.json`'s devDependencies covers `playwright`) navigating the real site's dropdowns - direct deep-links get rejected ("This Activity/Values Not Allowed"), the real flow is home page → Blood Availability → state/district selects (Ant Design comboboxes, need `type()` + `Enter`, not `.fill()` + click, to actually register a selection) → Search.

**The mistake**: the search page defaults to component = "Packed Red Blood Cells", not "Whole Blood" (the only component this platform's schema supports, `lib/db/bank-stock.ts`'s hardcoded `COMPONENT`). First pass read the summary row without changing that filter, and entered T.S.S. Hospital as entirely empty ("Not Available" was true for PRBC, not Whole Blood) and Pandit General's *PRBC* figures into the `whole_blood` column. **The project owner caught it** by screenshotting the real site's expanded per-bank stock tables (which show all three components at once) - at which point it was independently re-confirmed via a fresh Playwright run expanding each bank's own "View Stock Availability" row, and the DB corrected via an upsert (`on conflict (bank_id, blood_group, component) do update`) to the real Whole Blood figures: T.S.S. Hospital A+:1/B+:3/AB+:1/O+:4/O-:2, Pandit General A+:1/B+:1/AB+:1/O+:3. **Lesson for next time this kind of external-source-to-DB task comes up: confirm which sub-category/filter/component a summary figure actually reflects before trusting it, especially when the target schema is narrower than the source's own categories** - the source data was never wrong, the component-filter mismatch was introduced by not checking what was actually selected.
